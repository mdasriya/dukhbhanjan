...
export const addCartProduct = (product) => (dispatch) => {
  ...
  return  axios.post("https://dull-gray-jackrabbit-shoe.cyclic.app//cart/create", product,{
   headers: {
...
export const getCartProduct = () => (dispatch) => {
   ...
   dispatch({ type: CART_PRODUCT_REQUEST })
  return axios.get("https://dull-gray-jackrabbit-shoe.cyclic.app/cart",{
      headers: {
         "Content-Type": "application/json",
         "Authorization": `Bearer ${localStorage.getItem("token")}`
       }
...
//   return axios.patch(`https://dull-gray-jackrabbit-shoe.cyclic.app//product/${id}`, dataobj)
//       .then((res) => {
...
// }
 ...
 export const handleCartDelete = (id) => (dispatch) => {
   dispatch({type:CART_PRODUCT_REQUEST})
  return  axios.delete(`https://dull-gray-jackrabbit-shoe.cyclic.app/cart/delete${id}`)
   .then((res) => {
