...
export const addCartProduct = (product) => (dispatch) => {
  ...
  return  axios.post("https://vast-teal-abalone-wrap.cyclic.app/cart/create", product,{
   headers: {
...
export const getCartProduct = () => (dispatch) => {
   ...
   dispatch({ type: CART_PRODUCT_REQUEST })
  return axios.get("https://vast-teal-abalone-wrap.cyclic.app/cart",{
      headers: {
         "Content-Type": "application/json",
         "Authorization": `Bearer ${localStorage.getItem("token")}`
       }
...
//   return axios.patch(`https://vast-teal-abalone-wrap.cyclic.app//product/${id}`, dataobj)
//       .then((res) => {
...
// }
 ...
 export const handleCartDelete = (id) => (dispatch) => {
   dispatch({type:CART_PRODUCT_REQUEST})
  return  axios.delete(`https://vast-teal-abalone-wrap.cyclic.app//cart/delete${id}`)
   .then((res) => {
